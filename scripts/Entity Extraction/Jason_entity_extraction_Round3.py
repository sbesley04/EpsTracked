...
An event is any specific real-world action, occurrence, or state that is described \
or referenced in the email — not the email communication itself unless the email IS \
the relevant act (e.g. a direct threat, a directive, a confession).
Good events: a meeting that took place, a payment made, a trip arranged, an alleged \
assault, a legal filing submitted, a person recruited, a statement made to press, \
a financial arrangement, a property transfer, a visit to a location.
Avoid: generic summaries ("they communicated"), redundant restatements of the same act, \
or events with no specific detail.
Be concrete: name the specific people, places, amounts, or actions involved. \
One action = one event. Do not bundle multiple distinct actions into one record.

Use actor = the person/org who initiated or drove the action.
Use recipient = the person/org who was targeted, affected, or received the action.

For raw_date_string: copy the EXACT date text from the email that describes WHEN \
THE EVENT OCCURRED (not necessarily when the email was sent). Use null if no \
event-specific date is mentioned.
...
{{
  "events": [
    {{
      ...
      "description":         "Specific, concrete description. Name exact people, places, amounts. Cite key phrases from the text.",
      "quoted_evidence":     "most relevant direct quote from the email",
      "raw_date_string":     "exact date text describing when the event occurred, or null",
      "raw_location_string": "exact location text from the email, or null"
    }}
  ]
}}
...
For each event produce TWO separate scores:

trafficking_likelihood  (0.0 – 1.0):
  How likely is it that this event is related to human trafficking or sexual \
exploitation? Score based on the content of the event itself.
  0.0 = definitely not related | 1.0 = explicit trafficking activity

judgement_confidence  (0.0 – 1.0):
  How confident are YOU in your trafficking_likelihood score? \
This reflects the clarity of the evidence, not the severity.
  Low confidence = ambiguous language, missing context, could be interpreted \
multiple ways.
  High confidence = clear, unambiguous evidence supporting your assessment \
(either way).
...
{{
  "flagged_events": [
    {{
      ...
      "flag_reasoning":         "1-2 sentence explanation of the likelihood score AND why you are or are not confident"
    }}
  ]
}}"""
