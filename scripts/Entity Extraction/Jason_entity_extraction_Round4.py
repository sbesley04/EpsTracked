...
RULES:
1. Jeffrey Epstein is the ANCHOR entity. He may appear as ANY of these forms — \
treat them ALL as the same person with ID ENT_000001:
   Full name variants: "Jeffrey Epstein", "Jeffery Epstein", "Jeffrey E.", \
"Jeffery E.", "Jeff Epstein", "J. Epstein"
   Short forms: "Epstein", "JE", "jee", "Jeffrey", "Jeff"
   Email addresses: "jeevacation@gmail.com", "jeeitunes@gmail.com", \
"jeffrey.epstein@gmail.com"
   Pronouns: "he", "him", "his" — when the referent is clearly Epstein
...
4. List EVERY surface form the entity appears as under "aliases".
5. Only include PERSON or ORGANIZATION entities that appear in relation to Epstein.
6. Do NOT include locations here.

ALREADY KNOWN ENTITIES (from previous documents — if you see these people/orgs, \
use the exact canonical_name shown so they get the same ID):
{known_entities}

Respond with ONLY valid JSON (no markdown):
{{
  "epstein_aliases_found": ["every surface form of Epstein seen in this document"],
  "entities": [
    {{
      "canonical_name": "Full Name (use the known name above if it matches)",
...
STAGE2_SYSTEM = (
    ...
    "Epstein case emails. You extract both what happened in the email AND what "
    "real-world events are described within any articles, filings, or media the "
    "email contains or forwards."
)

STAGE2_USER = """\
Extract all events involving Jeffrey Epstein and the entities listed below \
from the email thread, using TWO-LAYER extraction.
...
LAYER 1 — SURFACE EVENTS (what happened in/as this email):
Extract only if the email itself IS the relevant action — a directive, arrangement,
threat, legal statement, financial instruction, or substantive request.
DO NOT extract: plain email forwards with no added content, routine acknowledgements,
generic congratulations, or administrative replies. If an email does nothing but
forward an article with no commentary, skip Layer 1 entirely.

LAYER 2 — CONTENT EVENTS (what real-world events are described inside the email):
When an email contains a forwarded article, book excerpt, legal filing, or victim
account, extract the underlying events described within that content.
- Actor/recipient = the alleged real-world parties named in the content, NOT the
  publisher or person forwarding.
- Use event_type: reported_allegation for events from articles/filings/media.
- Use the subject line as an additional content source.

━━━ EVENT TYPES ━━━
Assign exactly one event_type per event:
  realized           — something that physically happened (meeting, assault, trip,
                       legal filing, statement to press, property transfer)
  communicative      — a substantive directive, arrangement, or threat made in/by
                       this email (not plain forwarding)
  financial          — payment, gift, property, or economic arrangement
  relational         — introduction of parties, establishment of ongoing relationship
  reported_allegation— event described inside a forwarded article, filing, or media
  concealment        — suppression of information, coaching on secrecy or private
                       browsing, reputation management tied to misconduct, instructions
                       to avoid paper trails

━━━ RULES ━━━
- Be concrete: name exact people, places, amounts, or actions. Cite key phrases.
- One action = one event. Do not bundle distinct actions into one record.
- Do not create redundant events for the same underlying act.
- actor = who initiated/drove the action; recipient = who was targeted/affected.
- raw_date_string: EXACT date text from the email for WHEN THE EVENT OCCURRED
  (not the email sent date). Null if no event-specific date is stated.
...
{{
  "events": [
    {{
      "event_id":            "EVT_001",
      "event_type":          "realized|communicative|financial|relational|reported_allegation|concealment",
      "layer":               1 or 2,
      "actor":               "canonical name",
      "actor_entity_id":     "ENT_XXXXXX or null",
      "recipient":           "canonical name",
      "recipient_entity_id": "ENT_XXXXXX or null",
      "description":         "Specific, concrete description citing key phrases from the text.",
      "quoted_evidence":     "most relevant direct quote",
      "raw_date_string":     "exact date text for when the event occurred, or null",
      "raw_location_string": "exact location text, or null"
    }}
  ]
}}
...
Email sent date (first message in thread): {email_date}

EVENTS TO NORMALIZE:
{events_json}

DATE NORMALIZATION — apply exactly one of these three cases per event:

CASE A — Clear event date exists in the email:
  A specific date, month/year, or approximate period is stated for WHEN THE EVENT \
OCCURRED (e.g. "on March 5th", "last Tuesday", "in 2001", "during the summer of 1999").
  → Normalize to ISO 8601: "YYYY-MM-DD", {{"start":"YYYY-MM-DD","end":"YYYY-MM-DD"}}, \
or {{"start":"YYYY-01-01","end":"YYYY-12-31"}} for decades/eras.
  → date_confidence: "exact" (specific day known), "approximate" (month/season/year known).

CASE B — Event clearly happened BEFORE the email was sent but no date given:
  The email describes something that already happened at an unspecified past time, \
AND the email date is clearly different from when the event occurred \
(e.g. "years ago", "back then", "when she was young", historical allegations).
  → date_normalized: null
  → date_confidence: "unknown"

CASE C — Event happened around the time the email was sent:
  The email describes an ongoing situation, an imminent plan, or something with no \
indication it happened at a different time.
  → Use the email sent date: "{email_date}" normalized to ISO 8601.
  → date_confidence: "inferred"

Location normalization: use "Venue, City, Country" or "City, Country" or null. \
Only use "inferred" confidence if the location is strongly implied but not stated.
...
STAGE4_USER = """\
You are reviewing Epstein case email events for human trafficking indicators.

━━━ STEP 1: THREAD-LEVEL ASSESSMENT ━━━
Before scoring individual events, read the FULL THREAD TEXT below and form a
thread-level prior on trafficking likelihood (0.0-1.0). This prior anchors your
per-event scores — no individual event may be scored more than 0.3 below the
thread prior without an explicit stated reason in flag_reasoning.

FULL THREAD TEXT:
{thread_text}

━━━ STEP 2: PER-EVENT SCORING ━━━
Score each event below for trafficking indicators.

PRIMARY TRAFFICKING SIGNALS:
...
- Scheduling visits/meetings with unnamed or suspicious participants
- Suspicious payments or gifts involving persons

CONCEALMENT & REPUTATION-MANAGEMENT SIGNALS (baseline score 0.3-0.5):
- Coaching on secrecy, private browsing, or avoiding paper trails
- Reputation management tied to allegations of misconduct
- Search suppression or media strategy related to exploitation allegations
- Instructions to destroy records or avoid documented communication

DENIAL / REBUTTAL RULE:
  If an event is a denial, legal rebuttal, or public statement contesting an
  allegation, score the UNDERLYING ALLEGATION BEING DENIED — not the act of
  denial itself. A denial does NOT lower trafficking_likelihood; if the
  allegation is credible, the denial may raise judgement_confidence.

FINANCIAL EVENTS IN CONTEXT:
  Score financial events (payments, transfers, gifts) in light of the full
  thread. A payment that co-occurs with procurement or transport of individuals
  should score significantly higher than an isolated business payment.
...
trafficking_likelihood (0.0-1.0):
  How likely is this event related to human trafficking or sexual exploitation?
  0.0 = definitely not | 1.0 = explicit trafficking activity

judgement_confidence (0.0-1.0):
  How confident are you in your trafficking_likelihood score?
  Low = ambiguous language, missing context, multiple plausible interpretations.
  High = clear, unambiguous evidence supporting your assessment (either direction).

trafficking_flag = true if trafficking_likelihood >= 0.5, else false.

Respond with ONLY valid JSON (no markdown):
{{
  "thread_assessment": {{
    "thread_trafficking_likelihood": 0.0 to 1.0,
    "thread_reasoning": "1-2 sentence summary of why the thread as a whole does or does not suggest trafficking"
  }},
  "flagged_events": [
    {{
      "event_id":               "EVT_001",
      "trafficking_flag":       true or false,
      "trafficking_likelihood": 0.0 to 1.0,
      "judgement_confidence":   0.0 to 1.0,
      "trafficking_indicators": ["specific phrases / patterns that triggered the assessment"],
      "flag_reasoning":         "1-2 sentence explanation of the likelihood score AND confidence"
    }}
  ]
}}"""
...
def extract_thread(
    ...
    # Build known-entities context for Stage 1 (inter-document linking hint)
    known_lines = []
    for eid, info in list(registry.id_to_info.items())[:80]:  # cap at 80 to save tokens
        if eid == EPSTEIN_ID:
            continue
        top_aliases = ", ".join(info.get("aliases", [])[:3])
        known_lines.append(f"  {info['canonical_name']} ({eid}) — also: {top_aliases}")
    ...
    try:
        s1, inp, out = call_api(
            ...
            STAGE1_SYSTEM,
            STAGE1_USER.format(
                known_entities=known_entities_ctx,
                subject=subject,
    ...
    try:
        s3, inp, out = call_api(
            client,
            STAGE3_SYSTEM,
            STAGE3_USER.replace("{email_date}", email_date)
    ...
    try:
        s4, inp, out = call_api(
            client,
            STAGE4_SYSTEM,
            STAGE4_USER.replace("{thread_text}", thread_text)
                        .replace("{events_json}", events_json),
            model=model, max_tokens=2000,
        )
        total_in += inp; total_out += out
        ...
        if qc:
            qc_display = {
                "thread_assessment": s4.get("thread_assessment", {}),
                "flagged_events": [
                    {"event_id":               fe.get("event_id"),
                     "trafficking_flag":        fe.get("trafficking_flag"),
                     "trafficking_likelihood":  fe.get("trafficking_likelihood"),
                     "judgement_confidence":    fe.get("judgement_confidence"),
...
def flatten_results(results: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for r in results:
        ...
        thread_tl = str(
            s4.get("thread_assessment", {}).get("thread_trafficking_likelihood", "")
        )
        ...
        if not events:
            rows.append({
                ...
                "event_id": "", "event_type": "", "layer": "",
                "actor": "", "actor_entity_id": "",
                "recipient": "", "recipient_entity_id": "",
                "event_description": "", "quoted_evidence": "",
                "date_case": "", "date_normalized": "", "date_confidence": "",
                "location_normalized": "", "location_confidence": "",
                "thread_trafficking_likelihood": thread_tl,
                "trafficking_flag": "",
                "trafficking_likelihood": "", "judgement_confidence": "",
        ...
        for ev in events:
            ...
            rows.append({
                ...
                "date_case":                    clean_text(norm.get("date_case", "")),
                ...
                "trafficking_likelihood":       str(flag.get("trafficking_likelihood", "")),
                "judgement_confidence":         str(flag.get("judgement_confidence", "")),
